...
type KuskHook struct {
	logPath string
	lock    *sync.Mutex
}

func newKuskHook(logPath string) *KuskHook {
	hook := &KuskHook{lock: new(sync.Mutex)}
	hook.logPath = logPath
	return hook
}

// Write a log line to an io.Writer.
func (hook *KuskHook) ioWrite(entry *logrus.Entry) error {
	module := "general"
...
func (hook *KuskHook) Fire(entry *logrus.Entry) error {
	hook.lock.Lock()
...
func (hook *KuskHook) Levels() []logrus.Level {
