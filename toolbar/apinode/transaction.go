...
func (n *Node) BatchSendKUSK(accountID, password string, outputs map[string]uint64, memo []byte) error {
	totalKUSK := uint64(10000000)
	actions := []interface{}{}
	if len(memo) > 0 {
		actions = append(actions, &RetireAction{
			Arbitrary:   memo,
			AssetAmount: &bc.AssetAmount{AssetId: consensus.KUSKAssetID, Amount: 1},
		})
	}
	...
	for address, amount := range outputs {
		actions = append(actions, &ControlAddressAction{
			Address:     address,
			AssetAmount: &bc.AssetAmount{AssetId: consensus.KUSKAssetID, Amount: amount},
		})
		totalKUSK += amount
	}
	...
	actions = append(actions, &SpendAccountAction{
		AccountID:   accountID,
		AssetAmount: &bc.AssetAmount{AssetId: consensus.KUSKAssetID, Amount: totalKUSK},
	})
