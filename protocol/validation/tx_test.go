...
func TestOverflow(t *testing.T) {
	...
	newTx := func(inputs []uint64, outputs []uint64) *bc.Tx {
		...
		for i, amount := range inputs {
			txInput := types.NewSpendInput(nil, *sourceID, *consensus.KUSKAssetID, amount, uint64(i), ctrlProgram, nil)
			txInputs = append(txInputs, txInput)
		}

		for _, amount := range outputs {
			txOutput := types.NewOriginalTxOutput(*consensus.KUSKAssetID, amount, ctrlProgram, nil)
...
func TestTxValidation(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			f: func() {
				addCoinbase(consensus.KUSKAssetID, 100000, nil)
			},
			err: nil,
		},
		{
		...
		},
		{
			...
			f: func() {
				...
				addCoinbase(consensus.KUSKAssetID, 100000, arbitrary)
			},
...
func TestCoinbase(t *testing.T) {
	...
	CbTx := types.MapTx(&types.TxData{
		...
		Outputs: []*types.TxOutput{
			types.NewOriginalTxOutput(*consensus.KUSKAssetID, 888, cp, nil),
		},
	})
			...
			block: &bc.Block{
				...
				Transactions: []*bc.Tx{
					...
					types.MapTx(&types.TxData{
						...
						Inputs: []*types.TxInput{
							types.NewCoinbaseInput(nil),
							types.NewSpendInput([][]byte{}, *newHash(8), *consensus.KUSKAssetID, 100000000, 0, cp, nil),
						},
						Outputs: []*types.TxOutput{
							types.NewOriginalTxOutput(*consensus.KUSKAssetID, 888, cp, nil),
							types.NewOriginalTxOutput(*consensus.KUSKAssetID, 90000000, cp, nil),
						},
					}),
				},
			},
			...
			block: &bc.Block{
				...
				Transactions: []*bc.Tx{
					types.MapTx(&types.TxData{
						...
						Inputs: []*types.TxInput{
							types.NewCoinbaseInput(nil),
							types.NewSpendInput([][]byte{}, *newHash(8), *consensus.KUSKAssetID, 100000000, 0, retire, nil),
						},
						Outputs: []*types.TxOutput{
							types.NewOriginalTxOutput(*consensus.KUSKAssetID, 888, cp, nil),
							types.NewOriginalTxOutput(*consensus.KUSKAssetID, 90000000, cp, nil),
...
func TestTimeRange(t *testing.T) {
	...
	tx := types.MapTx(&types.TxData{
		...
		Outputs: []*types.TxOutput{
			types.NewOriginalTxOutput(*consensus.KUSKAssetID, 1, []byte{0x6a}, nil),
		},
	})
...
func mockGasTxInput() *types.TxInput {
	cp, _ := vmutil.DefaultCoinbaseProgram()
	return types.NewSpendInput([][]byte{}, *newHash(8), *consensus.KUSKAssetID, 100000000, 0, cp, nil)
}
