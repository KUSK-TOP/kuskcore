...
func (m *mempool) GetTransactions() []*core.TxDesc {
	...
	for i := 0; i < txsNumber; i++ {
		txInput := types.NewSpendInput(nil, bc.NewHash([32]byte{0x01}), *consensus.KUSKAssetID, uint64(i), 1, []byte{0x51}, [][]byte{})
		...
		tx := &types.Tx{
			...
			TxData: types.TxData{
				...
				Outputs: []*types.TxOutput{
					types.NewOriginalTxOutput(*consensus.KUSKAssetID, uint64(i), []byte{0x6a}, [][]byte{}),
				},
				SerializedSize: 1000,
			},
			Tx: &bc.Tx{
