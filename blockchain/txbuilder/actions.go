...
func (a *registerAction) Build(ctx context.Context, b *TemplateBuilder) error {
	...
	if a.AssetId.String() != consensus.KUSKAssetID.String() {
		return errors.New("register contract action asset must be KUSK")
	}

	if a.Amount < consensus.BCRPRequiredKUSKAmount {
		return errors.New("less than BCRP required KUSK amount")
	}
