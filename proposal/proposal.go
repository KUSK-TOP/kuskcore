...
func (b *blockBuilder) createCoinbaseTx() (tx *types.Tx, err error) {
	...
	if err = builder.AddOutput(types.NewOriginalTxOutput(*consensus.KUSKAssetID, 0, script, [][]byte{})); err != nil {
		return nil, err
	}
	...
	if b.block.Height%consensus.ActiveNetParams.BlocksOfEpoch == 1 && b.block.Height != 1 {
		for controlProgram, amount := range checkpoint.Rewards {
			...
			if err := builder.AddOutput(types.NewOriginalTxOutput(*consensus.KUSKAssetID, amount, controlProgramBytes, [][]byte{})); err != nil {
				return nil, err
			}
		}
	}
