...
import (
	...
	chainjson "kuskcore/encoding/json"
	"kuskcore/errors"
	"kuskcore/protocol"
...
// DefaultNativeAsset native KUSK asset
...
func initNativeAsset() {
	...
	alias := consensus.KUSKAlias

	definitionBytes, _ := serializeAssetDef(consensus.KUSKDefinitionMap)
	DefaultNativeAsset = &Asset{
		Signer:            signer,
		AssetID:           *consensus.KUSKAssetID,
		Alias:             &alias,
		VMVersion:         1,
		DefinitionMap:     consensus.KUSKDefinitionMap,
...
// pre-define errors for supporting kusk errorFormatter
var (
...
// Asset describe asset on kusk chain
type Asset struct {
	*signers.Signer
...
func (reg *Registry) GetAliasByID(id string) string {
	//btm
	if id == consensus.KUSKAssetID.String() {
		return consensus.KUSKAlias
	}
...
func (reg *Registry) UpdateAssetAlias(id, newAlias string) error {
	...
	if oldAlias == consensus.KUSKAlias || newAlias == consensus.KUSKAlias {
