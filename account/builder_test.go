...
func TestReserveKuskUtxoChain(t *testing.T) {
	...
	for i, c := range cases {
		...
		utxos, err := m.reserveKuskUtxoChain(&txbuilder.TemplateBuilder{}, "TestAccountID", c.amount, false)
...
func TestBuildKuskTxChain(t *testing.T) {
	txbuilder.ChainTxUtxoNum = 3
	m := mockAccountManager(t)
	cases := []struct {
		inputUtxo  []uint64
	...
	for caseIndex, c := range cases {
		...
		tpls, gotUtxo, err := m.buildKuskTxChain(utxos, acct.Signer)
		if err != nil {
			t.Fatal(err)
		}
...
func TestMergeSpendAction(t *testing.T) {
	testKUSK := &bc.AssetID{}
	if err := testKUSK.UnmarshalText([]byte("ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff")); err != nil {
		t.Fatal(err)
	}
