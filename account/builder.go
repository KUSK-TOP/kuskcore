...
func (m *Manager) reserveKuskUtxoChain(builder *txbuilder.TemplateBuilder, accountID string, amount uint64, useUnconfirmed bool) ([]*UTXO, error) {
	...
	for gasAmount := uint64(0); reservedAmount < gasAmount+amount; gasAmount = calcMergeGas(len(utxos)) {
		...
		res, err := m.utxoKeeper.Reserve(accountID, consensus.KUSKAssetID, reserveAmount, useUnconfirmed, nil, builder.MaxTime())
		if err != nil {
			return nil, err
		}
...
func (m *Manager) buildKuskTxChain(utxos []*UTXO, signer *signers.Signer) ([]*txbuilder.Template, *UTXO, error) {
	if len(utxos) == 0 {
	...
	for index := 0; index < len(utxos); index++ {
		...
		output := types.NewOriginalTxOutput(*consensus.KUSKAssetID, outAmount, acp.ControlProgram, utxos[index].StateData)
...
func SpendAccountChain(ctx context.Context, builder *txbuilder.TemplateBuilder, action txbuilder.Action) ([]*txbuilder.Template, error) {
	...
	if *act.AssetId != *consensus.KUSKAssetID {
		return nil, errors.New("spend chain action only support KUSK")
	}

	utxos, err := act.accounts.reserveKuskUtxoChain(builder, act.AccountID, act.Amount, act.UseUnconfirmed)
	if err != nil {
		return nil, err
	}
	...
	tpls, utxo, err := act.accounts.buildKuskTxChain(utxos, acct.Signer)
	if err != nil {
		return nil, err
	}
	...
	if utxo.Amount > act.Amount {
		if err = builder.AddOutput(types.NewOriginalTxOutput(*consensus.KUSKAssetID, utxo.Amount-act.Amount, utxo.ControlProgram, utxo.StateData)); err != nil {
