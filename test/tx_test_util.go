...
func txFee(tx *types.Tx) uint64 {
	...
	for _, input := range tx.Inputs {
		if input.AssetID() == *consensus.KUSKAssetID {
			inputSum += input.Amount()
		}
	}

	for _, output := range tx.Outputs {
		if *output.AssetId == *consensus.KUSKAssetID {
...
func CreateCoinbaseTx(controlProgram []byte, height, txsFee uint64) (*types.Tx, error) {
	...
	if err := builder.AddOutput(types.NewOriginalTxOutput(*consensus.KUSKAssetID, coinbaseValue, controlProgram, nil)); err != nil {
		return nil, err
	}
...
func CreateTxFromTx(baseTx *types.Tx, outputIndex uint64, outputAmount uint64, ctrlProgram []byte) (*types.Tx, error) {
	...
	output := types.NewOriginalTxOutput(*consensus.KUSKAssetID, outputAmount, ctrlProgram, nil)
	builder := txbuilder.NewBuilder(time.Now())
...
func CreateRegisterContractTx(contract []byte) (*types.Tx, error) {
	txInput := types.NewSpendInput(nil, bc.NewHash([32]byte{0x01}), *consensus.KUSKAssetID, 200000000, 1, []byte{0x51}, nil)
	...
	output := types.NewOriginalTxOutput(*consensus.KUSKAssetID, 100000000, program, [][]byte{})
	builder := txbuilder.NewBuilder(time.Now())
	if err := builder.AddInput(txInput, &txbuilder.SigningInstruction{}); err != nil {
		return nil, err
	}
...
func CreateUseContractTx(hash []byte, arguments [][]byte, stateData [][]byte) (*types.Tx, error) {
	...
	txInput := types.NewSpendInput(nil, bc.NewHash([32]byte{0x01}), *consensus.KUSKAssetID, 200000000, 1, program, stateData)
